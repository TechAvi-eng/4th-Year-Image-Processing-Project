function [precision, recall, ap] = calculate_pr_curve(pred_boxes, pred_scores, gt_boxes, iou_threshold)
% CALCULATE_PR_CURVE Calculates precision-recall curve and plots it
%
% INPUTS:
...
%
% Examples:
%   % Calculate and plot PR curve with IoU threshold 0.5
%   pred_boxes = [10, 10, 50, 50; 20, 20, 40, 40; 200, 200, 30, 30];
%   pred_scores = [0.9; 0.8; 0.7];
%   gt_boxes = [12, 12, 48, 48; 210, 210, 25, 25];
%   [precision, recall, ap50] = calculate_pr_curve(pred_boxes, pred_scores, gt_boxes);
%
%   % Calculate and plot PR curve with IoU threshold 0.75
%   [precision, recall, ap75] = calculate_pr_curve(pred_boxes, pred_scores, gt_boxes, 0.75);
    ...
    if isempty(pred_boxes) || isempty(gt_boxes)
        ...
        recall = 0;
        ap = 0;
        % Plot an empty PR curve
        figure;
        plot(0, 0, 'r-');
        title(['Precision-Recall Curve (IoU = ' num2str(iou_threshold) ', AP = 0)']);
        xlabel('Recall');
        ylabel('Precision');
        axis([0 1 0 1]);
        grid on;
    ...
    title(['Precision-Recall Curve (IoU = ' num2str(iou_threshold) ', AP = ' num2str(ap, '%.4f') ')']);
    xlabel('Recall');
    ylabel('Precision');
